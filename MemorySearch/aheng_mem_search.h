...
#ifndef RXMEMSCAN_aheng_mem_search_MINI_H
#define RXMEMSCAN_aheng_mem_search_MINI_H
...
class rx_region {
	...
	{
		return sizeof(vm_address_t) + sizeof(vm_address_t) +
		       sizeof(uint32_t) + compressed_region_data_size +
		       (matched_offs ?
				(sizeof(matched_off_t) * matched_offs->size()) :
				0);
	}
};
...
template <typename T> class rx_comparator_typed_eq : public rx_comparator {
	boolean_t compare(void *a, void *b)
	{
		return *(T *)b == *(T *)a;
	}
};

template <typename T> class rx_comparator_typed_ne : public rx_comparator {
	boolean_t compare(void *a, void *b)
	{
		return *(T *)b != *(T *)a;
	}
...
template <typename T> class rx_comparator_typed_lt : public rx_comparator {
	boolean_t compare(void *a, void *b)
	{
		return *(T *)b < *(T *)a;
	}
...
template <typename T> class rx_comparator_typed_gt : public rx_comparator {
	boolean_t compare(void *a, void *b)
	{
		return *(T *)b > *(T *)a;
	}
...
class rx_search_typed_value_type : public rx_search_value_type {
	...
	{
		switch (ct) {
		case rx_compare_type_eq:
			return new rx_comparator_typed_eq<T>();
		case rx_compare_type_ne:
			return new rx_comparator_typed_ne<T>();
		case rx_compare_type_lt:
			return new rx_comparator_typed_lt<T>();
		case rx_compare_type_gt:
			return new rx_comparator_typed_gt<T>();
		default:
			return NULL;
		}
	}
...
class aheng_mem_search {
	...
	rx_memory_page_pt page_of_memory(vm_address_t address,
					 uint32_t page_size);
    ...
    private:
	matched_off_t offset_of_matched_offsets(matched_offs_t &vec,
						uint32_t off_idx);
	inline void free_region_memory(region_t &region);
	inline void matched_offs_flush(matched_offs_context_t &ctx,
				       matched_offs_t &vec);
	inline void add_matched_offs_multi(matched_offs_t &vec,
					   matched_off_t bi, matched_off_ct ic);
	inline void add_matched_off(matched_off_t matched_idx,
				    matched_offs_context_t &ctx,
				    matched_offs_t &vec);
	inline kern_return_t read_region(data_pt region_data, region_t &region,
...
#endif //RXMEMSCAN_aheng_mem_search_MINI_H
