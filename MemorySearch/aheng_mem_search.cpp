#include "aheng_mem_search.h"

#include <mm.h>
#include <mm_types.h>
#include "lz4/lz4.h"
#include <cstddef>
#include "E:/Microsoft/AndroidNDK64/android-ndk-r16b/sysroot/usr/include/sys/time.h"
#include "E:/Microsoft/AndroidNDK64/android-ndk-r16b/sysroot/usr/include/sys/ptrace.h"
#include "E:/Microsoft/AndroidNDK64/android-ndk-r16b/sysroot/usr/include/linux/ptrace.h"
#include <string.h>
...
aheng_mem_search::aheng_mem_search() {
    _idle                   = true;
    _regions_p              = NULL;
...
void aheng_mem_search::free_memory() {
    free_regions();
...
void aheng_mem_search::reset() {
    free_memory();
    init_regions();
    _idle = true;
}

boolean_t aheng_mem_search::attach(pid_t pid) {
    _target_pid = pid;
    kern_return_t ret = ptrace(PTRACE_ATTACH, pid);
    //kern_return_t ret = task_for_pid(mach_task_self(), pid, &_target_task);
    if (ret == -1) {
        _trace("Attach to: %d Failed: %d, errno: %d\n", pid, ret, errno);
        printf("Attach to: %d Failed: %d, errno: %d\n", pid, ret, errno);
        return false;
    }
    reset();
    return true;
...
pid_t aheng_mem_search::target_pid() {
    return _target_pid;
}

mach_port_t aheng_mem_search::target_task() {
    return _target_task;
}

boolean_t aheng_mem_search::is_idle() {
    return _idle;
}

search_result_t aheng_mem_search::first_fuzzy_search() {
    return search(rx_search_fuzzy_val, rx_compare_type_void);
}

search_result_t aheng_mem_search::fuzzy_search(rx_compare_type ct) {
    return search(rx_search_fuzzy_val, ct);
}

search_result_t aheng_mem_search::last_search_result() {
    return _last_search_result;
}

void aheng_mem_search::set_search_value_type(rx_search_value_type *type_p) {
...
rx_memory_page_pt aheng_mem_search::page_of_memory(vm_address_t address, uint32_t page_size) {
...
rx_memory_page_pt aheng_mem_search::page_of_matched(uint32_t page_no, uint32_t page_size) {
...
kern_return_t aheng_mem_search::write_val(vm_address_t address, search_val_pt val) {
...
void aheng_mem_search::set_last_search_val(search_val_pt new_p) {
    if (!_last_search_val_p) {
...
search_result_t aheng_mem_search::search(search_val_pt search_val_p, rx_compare_type ct) {
...
matched_off_t aheng_mem_search::offset_of_matched_offsets(matched_offs_t &vec, uint32_t off_idx) {
...
void aheng_mem_search::free_regions() {
    if (_regions_p) {
        for (uint32_t i = 0; i < _regions_p->size(); ++i) {
            region_t region = (*_regions_p)[i];
            free_region_memory(region);
...
inline void aheng_mem_search::free_region_memory(region_t &region) {
...
inline void aheng_mem_search::matched_offs_flush(matched_offs_context_t &ctx, matched_offs_t &vec) {
...
inline void aheng_mem_search::add_matched_offs_multi(matched_offs_t &vec, matched_off_t bi, matched_off_ct ic) {
 ...
 */
inline void aheng_mem_search::add_matched_off(matched_off_t matched_idx, matched_offs_context_t &ctx, matched_offs_t &vec) {
    if (ctx.bf_nn) {
...
void aheng_mem_search::init_regions() {
    //struct proc_regioninfo region_info;
    vm_area_struct region_info;
    kern_return_t ret;
    uint64_t addr = 0;
    int count = 0;
...
void aheng_mem_search::init_regions() {
    ...
    do {
        if (addr) {
		    boolean_t writable = (vm_area.vm_flags == VM_READ ||
					  vm_area.vm_flags == VM_WRITE ||
					  vm_area.vm_flags == VM_EXEC);

            if (writable) {
                region_t region;
                region.address = vm_area.vm_start;
                region.size =(vm_size_t)(vm_area.vm_end - vm_area.vm_start);
                _regions_p->push_back(region);
                count ++;
            }
        }

        ret = proc_pidinfo(_target_pid, PROC_PIDREGIONINFO, addr, &vm_area, sizeof(vm_area));
	addr = vm_area.vm_end;
    } while (ret == sizeof(vm_area));
    _trace("Writable region count: %d\n", (int)_regions_p->size());
}
